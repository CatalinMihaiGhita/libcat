...
class susp_cell : public cell<flatten_or_void_t<lazy, std::invoke_result_t<F,T>>>
{
public:
    susp_cell(F &&f)
        : f(std::forward<F>(f))
    ...
    void take(void* t) override
    {
        if constexpr (std::is_void_v<flatten_or_void_t<lazy, std::invoke_result_t<F,T>>>) {
            f(*static_cast<T*>(t));
        } else {
...
template <typename T>
struct is_monad<lazy<T>> : public std::true_type
{
    using flat_type = T;
};
